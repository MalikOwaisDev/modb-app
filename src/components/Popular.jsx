...
const EndMessage = memo(({ category }) => (
  <div className="text-center p-4 sm:p-8 text-zinc-500 text-sm sm:text-base">
    <p>
      You've seen all popular {category === "movie" ? "movies" : "TV shows"}
    </p>
  </div>
));
...
function Popular() {
  ...
  // Fetch popular content - memoized for performance
  const getPopular = useCallback(async () => {
    ...
    try {
      ...
      setError("Failed to load popular content. Please try again later.");
    } finally {
      setIsLoading(false);
    }
  ...
  // Fetch initial data when page or popular array changes
  useEffect(() => {
    if (page === 1 && popular.length === 0) {
      getPopular();
    }
  }, [page, popular.length, getPopular]);

  // Add keyframes for animations - only once
  useEffect(() => {
    if (!document.getElementById("popular-animations")) {
      const style = document.createElement("style");
      style.id = "popular-animations";
      style.textContent = `
        @keyframes float {
          0% {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Header Section */}
        <motion.div
          className="top-0 z-10 bg-zinc-900/80 backdrop-blur-md border-b border-zinc-800/50 px-3 sm:px-6 py-3 sm:py-4 relative"
          initial={{ opacity: 0, y: -20 }}
        ...
        >
          ...
          {!isMobile && !isTablet && (
            <div
              className="absolute -top-20 -right-20 w-96 h-96 rounded-full bg-[#00b1b3]/5 blur-3xl"
              style={{
          ...
          <div
            className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 z-60 relative"
            style={{
              transform:
                !isMobile && !isTablet
          ...
          >
            ...
            <div
              className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto mt-2 sm:mt-0"
              style={{
                transform: !isMobile && !isTablet ? "translateZ(25px)" : "none",
              }}
            >
              <div className="relative group w-32 sm:w-auto">
                <Dropdown
                  title="Category"
                  options={["movie", "tv"]}
              ...
              <motion.button
                onClick={refreshHandler}
                className="p-2 rounded-full bg-zinc-800/80 hover:bg-[#00b1b3] transition-all text-white overflow-hidden group relative"
        ...
        {error ? (
          ...
          <div className="px-3 sm:px-6 py-3 sm:py-4">
            <InfiniteScroll
              ...
              hasMore={hasMore}
              loader={<Loader />}
              endMessage={<EndMessage category={category} />}
